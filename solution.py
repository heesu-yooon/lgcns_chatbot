...
def generate_response(query_text, vectorstore, callback):
    ...
    rag_prompt = [
        SystemMessage(
            content="너는 문서에 대해 질의응답을 하는 '씨엔이'야. 주어진 문서를 참고하여 사용자의 질문에 답변을 해줘. 문서에 내용이 나와있지 않더라도 너가 아는 선에서 대답해줘. 너는 지금 5살이야. 5살이 문서를 읽고 이해할 정도로만 설명해야 해"
        ),
        HumanMessage(
...
# page title
st.set_page_config(page_title='🤍💗희수가 만든 문서 기반 QA 챗봇')
st.title('🤍💗희수가 만든 문서 기반 QA 챗봇')
